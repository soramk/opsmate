        ...
        return `
            <div class="tool-panel">
                ...
                <div class="panel-card mt-6">
                    ...
                    <pre id="excel-output" class="bg-slate-950 p-4 rounded-lg font-mono text-sm text-emerald-400 overflow-auto"></pre>
                    <div id="excel-desc" class="text-xs text-slate-400 mt-3 p-3 bg-slate-900/50 rounded-lg"></div>
                </div>
                ...
                <div class="panel-card mt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div class="bg-slate-900 p-3 rounded-lg cursor-pointer hover:border-emerald-500/50 border border-slate-800" onclick="OpsMateHelpers.copyToClipboard('=TRIM(CLEAN(A1))')">
                            <div class="text-xs text-slate-500 mb-1">空白・制御文字を除去</div>
                            <code class="text-xs text-emerald-400">=TRIM(CLEAN(A1))</code>
                        </div>
                        <div class="bg-slate-900 p-3 rounded-lg cursor-pointer hover:border-emerald-500/50 border border-slate-800" onclick="OpsMateHelpers.copyToClipboard('=SUBSTITUTE(A1,CHAR(10),\", \")')">
                            <div class="text-xs text-slate-500 mb-1">改行をカンマに置換</div>
                            <code class="text-xs text-emerald-400">=SUBSTITUTE(A1,CHAR(10),", ")</code>
                        </div>
                        <div class="bg-slate-900 p-3 rounded-lg cursor-pointer hover:border-emerald-500/50 border border-slate-800" onclick="OpsMateHelpers.copyToClipboard('=TEXT(A1,\"yyyy/mm/dd\")')">
                            <div class="text-xs text-slate-500 mb-1">日付を文字列に変換</div>
                            <code class="text-xs text-emerald-400">=TEXT(A1,"yyyy/mm/dd")</code>
                        </div>
                        <div class="bg-slate-900 p-3 rounded-lg cursor-pointer hover:border-emerald-500/50 border border-slate-800" onclick="OpsMateHelpers.copyToClipboard('=IFERROR(VLOOKUP(A1,B:C,2,FALSE),\"\")')">
                            <div class="text-xs text-slate-500 mb-1">VLOOKUPエラー処理付き</div>
                            <code class="text-xs text-emerald-400">=IFERROR(VLOOKUP(A1,B:C,2,FALSE),"")</code>
                        </div>
                        <div class="bg-slate-900 p-3 rounded-lg cursor-pointer hover:border-emerald-500/50 border border-slate-800" onclick="OpsMateHelpers.copyToClipboard('=LEFT(A1,FIND(\"@\",A1)-1)')">
                            <div class="text-xs text-slate-500 mb-1">メールアドレスからユーザー名抽出</div>
                            <code class="text-xs text-emerald-400">=LEFT(A1,FIND("@",A1)-1)</code>
                        </div>
                        <div class="bg-slate-900 p-3 rounded-lg cursor-pointer hover:border-emerald-500/50 border border-slate-800" onclick="OpsMateHelpers.copyToClipboard('=MID(A1,FIND(\"/\",A1)+1,100)')">
                            <div class="text-xs text-slate-500 mb-1">区切り文字以降を抽出</div>
                            <code class="text-xs text-emerald-400">=MID(A1,FIND("/",A1)+1,100)</code>
                        </div>
                    </div>
                </div>
